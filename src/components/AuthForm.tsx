'use client'

import { useSearchParams } from 'next/navigation'
import Link from 'next/link'
import { login, register, loginConGoogle } from '@/app/actions/auth'
import { Button } from '@/components/ui/button'
import { Input } from '@/components/ui/input'
import { Label } from '@/components/ui/label'

interface AuthFormProps {
  modo: 'login' | 'register'
}

export default function AuthForm({ modo }: AuthFormProps) {
  const searchParams = useSearchParams()
  const error = searchParams.get('error')

  const accion = modo === 'login' ? login : register

  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-8">

      {/* Error */}
      {error && (
        <div className="bg-red-950 border border-red-800 text-red-400 text-sm px-4 py-3 rounded-lg mb-6">
          {decodeURIComponent(error)}
        </div>
      )}

      {/* Botón Google */}
      <form action={loginConGoogle}>
        <button
          type="submit"
          className="w-full flex items-center justify-center gap-3 bg-white hover:bg-zinc-100 text-black font-medium py-2.5 rounded-lg transition-colors text-sm"
        >
          <GoogleIcon />
          Continuar con Google
        </button>
      </form>

      {/* Separador */}
      <div className="flex items-center gap-3 my-6">
        <div className="flex-1 h-px bg-zinc-800" />
        <span className="text-zinc-600 text-xs">o continúa con email</span>
        <div className="flex-1 h-px bg-zinc-800" />
      </div>

      {/* Formulario email/password */}
      <form action={accion} className="space-y-4">
        {modo === 'register' && (
          <div className="space-y-1.5">
            <Label htmlFor="nombre" className="text-zinc-400 text-sm">Nombre</Label>
            <Input
              id="nombre"
              name="nombre"
              placeholder="Tu nombre"
              required
              className="bg-zinc-800 border-zinc-700 text-white placeholder:text-zinc-600 focus:border-violet-500"
            />
          </div>
        )}

        <div className="space-y-1.5">
          <Label htmlFor="email" className="text-zinc-400 text-sm">Email</Label>
          <Input
            id="email"
            name="email"
            type="email"
            placeholder="tu@email.com"
            required
            className="bg-zinc-800 border-zinc-700 text-white placeholder:text-zinc-600 focus:border-violet-500"
          />
        </div>

        <div className="space-y-1.5">
          <Label htmlFor="password" className="text-zinc-400 text-sm">Contraseña</Label>
          <Input
            id="password"
            name="password"
            type="password"
            placeholder="••••••••"
            required
            minLength={6}
            className="bg-zinc-800 border-zinc-700 text-white placeholder:text-zinc-600 focus:border-violet-500"
          />
        </div>

        <Button
          type="submit"
          className="w-full bg-violet-600 hover:bg-violet-500 text-white font-medium mt-2"
        >
          {modo === 'login' ? 'Iniciar sesión' : 'Crear cuenta'}
        </Button>
      </form>

      {/* Link cambio de modo */}
      <p className="text-center text-zinc-500 text-sm mt-6">
        {modo === 'login' ? '¿No tienes cuenta?' : '¿Ya tienes cuenta?'}{' '}
        <Link
          href={modo === 'login' ? '/register' : '/login'}
          className="text-violet-400 hover:text-violet-300 transition-colors"
        >
          {modo === 'login' ? 'Regístrate' : 'Inicia sesión'}
        </Link>
      </p>
    </div>
  )
}

function GoogleIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 48 48">
      <path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/>
      <path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/>
      <path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24c0 3.88.92 7.54 2.56 10.78l7.97-6.19z"/>
      <path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/>
      <path fill="none" d="M0 0h48v48H0z"/>
    </svg>
  )
}